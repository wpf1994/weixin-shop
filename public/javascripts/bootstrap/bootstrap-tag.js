...
!function ( $ ) {
  ...
  , show: function () {
      ...
      that.input = $('<input type="text">')
        ...
        .on('keyup', function ( event ) {
          if ( event.keyCode != 188 && event.keyCode != 13 && event.keyCode != 9 ) {
              var v_length = $.trim($(this).val().length);
              if(that.options.auto_input_size > 0 && v_length > 0 && v_length >= that.options.auto_input_size){
                  that.process();
              }
          }
        })
  ...
  , process: function () {
      ...
      $.each(values, function() {
        //有自定义数据选择
        if ( that.options.validate && typeof that.options.validate === 'function' ){
          if(that.options.validate( values )) that.add(this)
        }else{
          that.add(this)
        }
      })
  ...
  , placeholder: ''
  , source: []
  , validate: null//自定义校验输入值是否合法，值为function(value){return true}
  , auto_input_size: 0//自定义固定长度后自动输入,值为数字
  }
